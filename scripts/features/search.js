...
async function applySort() {
    ...
    const itemsData = Object.entries(rawData).map(([id, value]) => ({ id, value }));
    ...
    if (params === "price (lowest to highest)") {
        sortItems((a, b) => {
            return parseFloat(a.value.price) - parseFloat(b.value.price);
        });
    }

    if (params === "price (highest to lowest)") {
        sortItems((a, b) => {
            return parseFloat(b.value.price) - parseFloat(a.value.price);
        });
    }
}
